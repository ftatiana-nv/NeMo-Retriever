...
from nemo_retriever.tabular_data.ingestion.graph.model.query import (
    Query,
    MissingDataError,
    NoFKError,
    handle_node_update,
...
from nemo_retriever.tabular_data.ingestion.graph.parsers.sql.op_name_to_symbol import get_symbol
from nemo_retriever.tabular_data.ingestion.graph.parsers.sql.utils import get_key_recursive
import pendulum
...
def create_binary_op_node(
    ...
    if binaryop_dict["op"] == "Eq" and parent.get_name() in [
        ...
        if left_node.get_label() == Labels.COLUMN and right_node.get_label() == Labels.COLUMN:
...
def create_column_node(
    ...
    if column_dict == Parser.WILDCARD:
        ...
        sql_part = ""
        if label_of_current_node == Labels.COLUMN:
            sql_part = col_name
            if identifier_key == "CompoundIdentifier":
...
def filter_alias_chains(
    ...
    allowed_leafs = [
        node
        for node in g.nodes
        if g.nodes[node]["node"].label in [Labels.COLUMN, Labels.TABLE]
    ]
    for n in g.nodes:
        ...
        if node_label not in [Labels.SQL, Labels.COLUMN, Labels.TABLE]:
            if (
    ...
    leaf_labels = [Labels.COLUMN, Labels.TABLE]
    for edge in g.edges():
...
def connect_and_join_tables(
    ...
    connected_columns_ids = []
    for n in g.nodes:
        if g.nodes[n]["node"].label == Labels.TABLE:
            tables_ids.append(n)
        elif g.nodes[n]["node"].label == Labels.COLUMN:
            column_ids.append(n)
    ...
    for table_column in tables_ids + column_ids:
        ...
        if g.nodes[table_column]["node"].label == Labels.COLUMN:
