...
This returns the complete list of tables you can query. When multiple DuckDB schemas are exposed, keys are `schema.table`; use the **qualified** `CREATE TABLE "schema"."table"` form from `sql_db_schema` when writing SQL.
