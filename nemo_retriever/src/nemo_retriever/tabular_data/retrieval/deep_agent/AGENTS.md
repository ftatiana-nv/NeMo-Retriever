...
- Schemas and tables: obtained dynamically using the available SQL tools. The runtime may also inject a **catalog system prompt** (all table keys, optionally full DDL) from the real DuckDB — configured in code as ``DEEP_AGENT_SQL_CATALOG_MODE`` in ``deep_agent_runtime.py``. **Never invent** table names from the question wording unless they resolve via ``sql_db_list_tables`` / ``sql_db_schema`` (canonical keys are ``schema.table``).
