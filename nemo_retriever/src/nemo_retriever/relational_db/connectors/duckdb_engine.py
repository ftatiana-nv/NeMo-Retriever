...
class DuckDBEngine:
    ...
    ----------
    database:
        Path to a persistent ``.duckdb`` or ``.sqlite`` file, or ``None`` /
        ``":memory:"`` for an ephemeral in-memory database (default: in-memory).
        ``.sqlite`` files are opened via DuckDB's built-in SQLite scanner
        (``ATTACH … TYPE SQLITE``); no conversion is needed.
    read_only:
    ...
    def __init__(self, connection: Optional[Dict[str, Any]] = None) -> None:
        ...
        db_path = str(self.connection_properties.get("database", ":memory:"))
        read_only = bool(self.connection_properties.get("read_only", False))

        if db_path.endswith(".sqlite"):
            # Connect in-memory and attach the SQLite file via DuckDB's sqlite extension.
            self.conn = duckdb.connect(":memory:")
            self.conn.execute("INSTALL sqlite; LOAD sqlite;")
            ro_clause = ", READ_ONLY" if read_only else ""
            self.conn.execute(f"ATTACH '{db_path}' AS db (TYPE SQLITE{ro_clause})")
            logger.debug("DuckDB attached SQLite (path=%r, read_only=%s).", db_path, read_only)
        else:
    ...
    @classmethod
    def from_file(cls, path: Union[str, Path], *, read_only: bool = True) -> "DuckDBEngine":
        """Create a DuckDBEngine from a file path, auto-detecting .sqlite vs .duckdb format."""
        return cls({"database": str(path), "read_only": read_only})
    ...
    def set_schema(self, schema: str) -> None:
        """Set the active search path / default schema for subsequent queries.

        Equivalent to ``SET search_path = schema`` in DuckDB so that unqualified
        table names (e.g. ``SELECT * FROM flights``) resolve against *schema*.
        """
        self.conn.execute(f"SET search_path = '{schema}'")
        logger.debug("DuckDB search_path set to %r.", schema)

    def execute_to_csv(self, sql: str, out_path: str, chunksize: int = 500) -> None:
        """Execute a query and write results to a CSV file in chunks.
        ...
        chunksize:
            Number of rows per write chunk (default: 500).
        """
        df = self.execute(sql)
        os.makedirs(os.path.dirname(os.path.abspath(out_path)), exist_ok=True)
        for i, chunk_start in enumerate(range(0, max(len(df), 1), chunksize)):
            chunk = df.iloc[chunk_start: chunk_start + chunksize]
            chunk.to_csv(out_path, mode="a" if i > 0 else "w", header=i == 0, index=False)
